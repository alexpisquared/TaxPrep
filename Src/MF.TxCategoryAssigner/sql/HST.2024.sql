USE [FinDemo]
go -- SELECT c.*, TxCategory.Name AS Ctg FROM TxCoreV2 c INNER JOIN TxCategory ON c.TxCategoryIdTxt = TxCategory.IdTxt WHERE (c.TxMoneySrcId = 3) AND (c.TxAmount < 0) AND (c.TxCategoryIdTxt <> 'BankFee') AND (c.TxDate >= CONVERT(DATETIME, '2024-01-01 00:00:00', 102)) AND (c.TxDate < CONVERT(DATETIME, '2024-04-01 00:00:00', 102)) ORDER BY c.TxDate DESC
