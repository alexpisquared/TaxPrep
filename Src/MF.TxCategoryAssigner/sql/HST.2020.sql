...
2. Dnld txns and run query C:\c\Live\MinFin\MF.TxCategoryAssigner\sql\HST.2020.sql
3. Compare/match/fixup the results */
USE [FinDemo]
go
--SELECT c.*, TxCategory.Name AS Ctg FROM TxCoreV2 c INNER JOIN TxCategory ON c.TxCategoryIdTxt = TxCategory.IdTxt WHERE (c.TxMoneySrcId = 3) AND (c.TxAmount < 0) AND (c.TxCategoryIdTxt <> 'BankFee') AND (c.TxDate >= CONVERT(DATETIME, '2020-01-01 00:00:00', 102)) AND (c.TxDate < CONVERT(DATETIME, '2020-04-01 00:00:00', 102)) ORDER BY c.TxDate DESC
declare @q0 as DATETIME, @q1 as DATETIME, @q2 as DATETIME, @q3 as DATETIME, @q4 as DATETIME; 
select @q0 = CONVERT(DATETIME, '2020-01-01 00:00:00', 102), @q1 = CONVERT(DATETIME, '2020-04-01 00:00:00', 102), @q2 = CONVERT(DATETIME, '2020-07-01 00:00:00', 102), @q3 = CONVERT(DATETIME, '2020-10-01 00:00:00', 102), @q4 = CONVERT(DATETIME, '2021-01-01 00:00:00', 102)
