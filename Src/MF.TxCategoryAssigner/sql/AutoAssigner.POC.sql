...
BEGIN
	...
	SELECT   SUBSTRING(TxDetail, 1, @MatchLen) AS TxDtl8, COUNT(*) AS Cnt, MIN(TxAmount) AS Min, MAX(TxAmount) AS Max, AVG(TxAmount) AS Avg, SUM(TxAmount) AS Sum, MIN(TxDate) AS FrstTxn, MAX(TxDate) AS LastTxn   FROM TxCoreV2 
	WHERE     (TxDate >= CONVERT(DATETIME, @YOI, 102))
