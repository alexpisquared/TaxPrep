...
public partial class PrintableReport
{
    ...
    async Task LoadReportDataAsync(string owner, FinDemoContext dba)
    {
        try
        {
            var taxYear = DateTime.Today.Year - 1;
            tbkCompany.Text = owner == "Alx" ? "AAVpro Ltd." : "Jing Mei Li";
            tbkTaxYear.Text = $"{taxYear} Tax Year";

            _viewModel.StatusText = "Loading report data...";

            // Get the report data
            IEnumerable<VwTaxLiqReport> data = await _reportService.GetTaxReportDataFromRawSqlAsync(owner, dba, new DateTime(taxYear, 1, 1), new DateTime(taxYear, 12, 31));
    ...
    void GenerateReport(IEnumerable<VwTaxLiqReport> reportData) => Dispatcher.Invoke(() =>
    {
        ...
        IOrderedEnumerable<IGrouping<string, VwTaxLiqReport>> groupedData = reportData.GroupBy(item => item.Group)
                .OrderBy(g => g.Min(i => i.TlNumber));

        double grandTotal = 0;

        foreach (IGrouping<string, VwTaxLiqReport>? group in groupedData)
        {
            // Add group header
            ...
            foreach (VwTaxLiqReport? item in group.OrderBy(i => i.TlNumber))
            {
