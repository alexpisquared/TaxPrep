...
SELECT -isnull(SUM(TxAmount)/4,0)	as [Revenue 1/4], (-isnull(SUM(TxAmount)/4,0))*.088-300 as HST	FROM TxCoreV2 WHERE (TxMoneySrcId = 3) AND (TxAmount < 0) AND TxDetail != @abr AND (TxDate >= @q0) AND (TxDate < @q1)                 -- +300 for the first quarter only!!!
SELECT -isnull(SUM(TxAmount)/4,0)	as [Revenue 2/4], (-isnull(SUM(TxAmount)/4,0))*.088     as HST	FROM TxCoreV2 WHERE (TxMoneySrcId = 3) AND (TxAmount < 0) AND TxDetail != @abr AND (TxDate >= @q0) AND (TxDate < @q1)                 
SELECT -isnull(SUM(TxAmount)/4,0)	as [Revenue 3/4], (-isnull(SUM(TxAmount)/4,0))*.088	    as HST	FROM TxCoreV2 WHERE (TxMoneySrcId = 3) AND (TxAmount < 0) AND TxDetail != @abr AND (TxDate >= @q0) AND (TxDate < @q1)                 
SELECT -isnull(SUM(TxAmount)/4,0)	as [Revenue 4/4], (-isnull(SUM(TxAmount)/4,0))*.088	    as HST	FROM TxCoreV2 WHERE (TxMoneySrcId = 3) AND (TxAmount < 0) AND TxDetail != @abr AND (TxDate >= @q0) AND (TxDate < @q1)                 
